...
class WallCost:
    ...
    def evaluate(self, x, u, x_center = 0.0, y_center = 0.0, side_length_x = 7.0, side_length_y = 7.0):
        ...
        dx = max(0, abs(x_robot - x_center) - 0.5 * side_length_x)
        dy = max(0, abs(y_robot - y_center) - 0.5 * side_length_y)

        dist_penalty = np.sqrt(dx**2 + dy**2)**2

        return self.weight * dist_penalty

    def gradient_x(self, x, u, x_center = 0.0, y_center = 0.0, side_length_x = 7.0, side_length_y = 7.0):
        x_robot = x[4 * self.idx]
        y_robot = x[4 * self.idx + 1]
    ...
    def hessian_x(self, x, u, x_center = 0.0, y_center = 0.0, side_length_x = 7.0, side_length_y = 7.0):
        x_robot = x[4 * self.idx]
        y_robot = x[4 * self.idx + 1]
